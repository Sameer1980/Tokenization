...
logging.basicConfig(filename=log_path + "/data_detoken_VTS.log", format=FORMAT,
...
def get_sql_data(i):
    ...
    password = retrive_pwd(credentials['sql-dev']['pwd_url'])
...
for num in range(1):
    ...
    try:
        ...
        print(e+'\nSkipping few records' )
        logging.info(e+'\nSkipping few records')
        df.loc[~df['token'].str.contains(r'\x01', case=True)].to_csv(r'{}'.format(filepath_tkn), index=False,
                                                                     header=['actual_data', 'tokenised_data'],
                                                                     sep='\x01', quoting=csv.QUOTE_NONE)
